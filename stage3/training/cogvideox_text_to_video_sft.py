...
# )
from cogvideox.autoencoder import AutoencoderKLCogVideoX
from cogvideox.scheduler import (
    CogVideoXDPMScheduler,
    CogVideoXSwinDPMScheduler,
    sample_timesteps_with_group,
)
from cogvideox.pipelines import CogVideoXPipeline, CogVideoXStreamingPipeline
from cogvideox.transformer import CogVideoXTransformer3DModel
from cogvideox.control_adapter import CONTROL_SIGNAL_TO_PROMPT
...
def log_validation(
    ...
    pipe: Union[CogVideoXPipeline, CogVideoXStreamingPipeline],
    args: Dict[str, Any],
    ...
    if accelerator.is_main_process:
        logger.info(
            f"Running validation... \n Generating {args.num_validation_videos} videos with prompt: {pipeline_args['prompt']}.\nControl: {pipeline_args['control_signal']}"
        )
    ...
    for i, video in enumerate(videos):
        ...
        video_id = f"{phase_name}_step_{step}_rank_{accelerator.process_index}_video_{i}.mp4"
        filename = os.path.join(args.output_dir, "videos", video_id)
        export_to_video(video, filename, fps=fps)
        prompt_id = f"{phase_name}_step_{step}_rank_{accelerator.process_index}_video_{i}.txt"
        prompt_filename = os.path.join(args.output_dir, "videos", prompt_id)
        with open(prompt_filename, "w", encoding="utf-8") as f:
            f.write(pipeline_args["prompt"])
            f.write("\n")
            write_prompt = ",".join(pipeline_args["control_signal"].split(",")[:(len(pipeline_args["control_signal"].split(","))+1)//3*2])  # skip padding control signal
            f.write(write_prompt)
        video_filenames.append(filename)

    for tracker in accelerator.trackers:
        # Only main process initialized accelerator.trackers
        if tracker.name == "wandb":
            tracker.log(
                {
                    phase_name: [
                        wandb.Video(filename, caption=f"{i}: {pipeline_args['prompt']};\n{pipeline_args['control_signal']}")
...
class CollateFunction:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        control_signal = [x["control_signal"] for x in data]
...
def main(args):
    ...
    # Synchronize output_dir to all processes.
    output_dir_to_broadcast = [args.output_dir]
    output_dir_to_broadcast = broadcast_object_list(output_dir_to_broadcast, from_process=0)
    args.output_dir = output_dir_to_broadcast[0]
    accelerator.project_configuration = ProjectConfiguration(project_dir=args.output_dir, logging_dir=logging_dir)
    ...
    transformer = CogVideoXTransformer3DModel.from_pretrained(
        ...
        control_dim=clip_model.text_embed_dim,
        control_start_layer=args.control_start_layer,
        control_end_layer=args.control_end_layer,
        control_zero_init=args.control_zero_init,
    ...
    if args.use_swin_dpm:
        extra_scheduer_kwargs = {
            "num_noise_groups": args.num_noise_groups,
            "num_sample_groups": args.num_sample_groups,
        }
        scheduler = CogVideoXSwinDPMScheduler.from_pretrained(
            args.pretrained_model_name_or_path,
            subfolder="scheduler",
            **extra_scheduer_kwargs,
        )
    ...
    train_dataloader = DataLoader(
        ...
        sampler=BucketSampler(train_dataset, global_batch_size=args.train_batch_size * accelerator.num_processes * args.gradient_accumulation_steps, shuffle=True),
    ...
    if not args.resume_from_checkpoint:
        ...
        skiped_train_dataloader = accelerator.skip_first_batches(train_dataloader, global_step % len(train_dataloader))
    ...
    # Embedding for each control signal
    ...
    control_prompt_ids.to(clip_model.device)
    control_embeddings = clip_model.get_text_features(**control_prompt_ids)
    null_control_prompt_ids = clip_tokenizer([""], padding=True, return_tensors="pt")
    null_control_prompt_ids.to(clip_model.device)
    null_control_embedding = clip_model.get_text_features(**null_control_prompt_ids)
    control_signal_to_idx = {k: i for i, k in enumerate(CONTROL_SIGNAL_TO_PROMPT.keys())}
    def get_control_from_signal(control_signal):
        control_indices = [control_signal_to_idx[c] for c in control_signal.split(",")]
        control = control_embeddings[control_indices]
        return control
    def get_null_control_from_signal(control_signal):
        control_signal = control_signal.split(",")
        null_control = null_control_embedding.repeat(len(control_signal), 1)
        return null_control
    ...
    while global_step < args.max_train_steps:
        ...
        with accelerator.accumulate(models_to_accumulate):
            ...
            # Get control signal
            control_emb = [
                get_null_control_from_signal(u) if np.random.random() < args.control_p_zero 
                    else get_control_from_signal(u) for u in control_signal
            ]
            control_emb = torch.stack(control_emb).to(weight_dtype).contiguous()
            ...
            if args.use_swin_dpm:
                timesteps_grouped = sample_timesteps_with_group(
                    batch_size,
                    num_frames,
                    num_noise_groups=args.num_noise_groups,
                ...
                timesteps = timesteps_grouped  # [B, F]
            else:
            ...
            if args.use_swin_dpm:
                loss = torch.mean(
                    (weights[:, 1:] * (model_pred[:, 1:] - target[:, 1:]) ** 2).reshape(batch_size, -1),
                    dim=1,
                )
            else:
                loss = torch.mean(
            ...
            if accelerator.is_main_process:
                with open(os.path.join(args.output_dir, "loss.log"), "a") as f:
                    f.write(f"[step {global_step}] lr: {last_lr}, loss: {loss.detach().item()}\n")
        ...
        if (global_step % args.validation_steps == 0) or (global_step == args.max_train_steps):
            ...
            validation_prompts = prompts[0:1]
            validation_control_signals = control_signal[0:1]
            for validation_prompt, validation_control_signal in zip(validation_prompts, validation_control_signals):
                # Padding control signal
                # First padding is for video exdention; second one is only for padding (with redundancy)
                validation_control_signal = validation_control_signal + "," + \
                    ",".join(validation_control_signal.split(",")[:0:-1]) + \
                    ",D"*len(validation_control_signal.split(","))
                pipeline_args = {
                    ...
                    "control_signal": validation_control_signal,
                    "num_frames": raw_num_frames
                }

                if args.use_swin_dpm:
                    pipeline_extra_args = {
                        "num_noise_groups": args.num_noise_groups,  # number of noise group number
                        "num_sample_groups": args.num_sample_groups,  # number of outer sampling loop, and each iteration output a group of video tokens.
                        "with_frame_cond": True,  # whether to use frame condition, if ture the first frame is used as condition with zero noise.
                        "num_inference_steps": 20,
                        "latents": videos.detach().clone()[:1],  # use batch input video as video initilization. [1, F, C, H, W] 
                    }
                    pipeline_args.update(pipeline_extra_args)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="save folder, e.g. 2025-01-13_11-57-29",
    )
    parser.add_argument(
        "--lr",
        type=float,
