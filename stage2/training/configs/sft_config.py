...
# Model Arguments
args.pretrained_model_name_or_path = "../models/stage1"  # str: Path to pretrained model or model identifier from huggingface.co/models.
...
args.index_file = "../data/0919_filtered_velocity_EN.json"
...
args.frame_buckets = [21, 65]  # list[int]: Must be 4*i+1; CogVideoX1.5 need to guarantee that ((num_frames - 1) // self.vae_scale_factor_temporal + 1) % patch_size_t == 0, such as 53.
...
args.output_dir = "output/sft/stage2/"+datetime.datetime.now(pytz.timezone('Asia/Shanghai')).strftime("%Y-%m-%d_%H-%M-%S")  # str: The output directory where the model predictions and checkpoints will be written.
