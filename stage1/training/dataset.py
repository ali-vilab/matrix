...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, meta) -> Dict[str, Any]:
        ...
        image, video, _ = self._preprocess_video(Path(os.path.join(self.data_root, fname)), resolution_id)
...
class VideoDatasetWithResizing(VideoDataset):
    ...
    def _preprocess_video(self, path: Path, resolution_id=0) -> torch.Tensor:
        video_reader = decord.VideoReader(uri=path.as_posix())
        video_num_frames = len(video_reader)
...
class VideoDatasetWithResizeAndRectangleCrop(VideoDataset):
    ...
    def _preprocess_video(self, path: Path, resolution_id=0) -> torch.Tensor:
        ...
        if video_num_frames < o_frame:
            ...
            frame_indices = np.array(list(range(video_num_frames))).repeat((o_frame // video_num_frames + 1))
            frame_indices = frame_indices[:o_frame]
        else:
            video_fps = video_reader.get_avg_fps()
            sampling_interval = video_fps/self.fps
