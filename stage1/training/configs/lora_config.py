...
# Model Arguments
args.pretrained_model_name_or_path = "../models/CogVideoX-2b"  # str: Path to pretrained model or model identifier from huggingface.co/models.
...
args.index_file = "../data/0830_pretrained_data_EN.json"
...
args.frame_buckets = [17, 33, 65]  # list[int]: CogVideoX1.5 need to guarantee that ((num_frames - 1) // self.vae_scale_factor_temporal + 1) % patch_size_t == 0, such as 53.
...
args.rank = 256  # int: The rank for LoRA matrices.
args.lora_alpha = 256  # int: The lora_alpha to compute scaling factor (lora_alpha / rank) for LoRA matrices.
...
args.output_dir = "output/lora/"+datetime.datetime.now(pytz.timezone('Asia/Shanghai')).strftime("%Y-%m-%d_%H-%M-%S")  # str: The output directory where the model predictions and checkpoints will be written.
...
args.tracker_name = None  # str: Project tracker name.
