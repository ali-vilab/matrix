    ...
    def __call__(
        ...
        **kwargs
        ...
        for group_idx in range(outer_steps):
            ...
            if do_classifier_free_guidance:
                null_control_emb = self.null_control_embedding.repeat(control_emb.size(1),1).unsqueeze(0).to(prompt_embeds.dtype).contiguous()
                control_emb = torch.cat([null_control_emb, control_emb], dim=0)
...
class CogVideoXStreamingPipelinePCFG(CogVideoXPipeline):
    def __init__(
        self,
        tokenizer: T5Tokenizer,
        text_encoder: T5EncoderModel,
